  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <p className="text-text-light mt-1">Acompanhe o andamento de suas compras parceladas (agrupadas pelo nome original).</p>
      </header>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {groupKeys.map(nomeKey => {
            const group = groupedParcelas[nomeKey];
            
            // Pega o registro mais recente (índice 0 pois está ordenado desc por data do supabase)
            const baseItem = group[0];
            
            // Lógica de Checklist: O progresso (parcela atual) é a quantidade de vezes que a transação foi importada
            ...
            return (
              <div key={nomeKey} className="glass-panel p-6 flex flex-col gap-4 relative overflow-hidden group/card border-t-4 border-t-transparent hover:border-t-primary transition-all">
                ...
                <div>
                  ...
                  <div className="text-sm text-primary mt-1 font-medium">{baseItem.banco || 'Desconhecido'}</div>
                </div>
                ...
                <div className="mt-auto pt-4 border-t border-border">
                  <div className="flex justify-between items-end mb-2">
                    ...
                    <div className="text-right">
                      ...
                      <span className={`font-bold text-lg ${current >= total ? 'text-primary' : 'text-text'}`}>
                        {current} de {total}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="w-full bg-border rounded-full h-2.5 overflow-hidden">
                    <div 
                      className={`h-2.5 rounded-full transition-all duration-500 ease-out ${current >= total ? 'bg-primary' : 'bg-primary/70'}`}
                  ...
                  <div className="mt-3 flex flex-col gap-1">
                    <div className="text-xs text-text-light flex justify-between items-center">
                      ...
                      <span className="font-medium text-primary">R$ {valorPago.toFixed(2)}</span>
                    </div>
                    <div className="text-xs text-text-light flex justify-between items-center">
                  ...
                  <button 
                    ...
                    className="mt-4 w-full flex items-center justify-center gap-1 text-xs font-bold text-primary hover:text-primary-hover transition-colors py-2 border-t border-border/50"
                  >
                  ...
                  {expandedGroups[nomeKey] && (
                    <div className="mt-2 flex flex-col gap-2 bg-primary/5 p-3 rounded-lg border border-primary/10">
                      <h4 className="text-[10px] font-bold text-text-light uppercase tracking-wider mb-1">Histórico de Pagamentos</h4>
                      {group.map((transacao, idx) => (
                        <div key={transacao.id} className="flex justify-between items-center text-xs border-b border-border/30 pb-1 last:border-0 last:pb-0">
                          ...
                          <span className="font-bold text-primary">R$ {Math.abs(Number(transacao.valor)).toFixed(2)}</span>
                        </div>
                      ))}
                    </div>
                  )}
